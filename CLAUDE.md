...
- **Gemini model choice + automatic fallback chain:** free-tier Gemini quota is **per-model** and small, so *any single model* can `429 RESOURCE_EXHAUSTED` mid-demo (observed live: `gemini-flash-latest` ran dry after some use). Rather than betting on one model, `advisor.py::get_advice` now tries the configured `GEMINI_MODEL` first and then **falls through `_FALLBACK_MODELS`** (`gemini-flash-lite-latest` → `gemini-3.5-flash` → `gemini-3.1-flash-lite` → `gemini-2.0-flash` → `gemini-flash-latest`) on a *retryable* error only — quota (`429`/`RESOURCE_EXHAUSTED`) or model-not-found (`404`/`NOT_FOUND`). A non-retryable error (bad key, malformed request) fails the same way on every model, so it's surfaced immediately instead of looping. Verified live by forcing the primary to the known-exhausted `gemini-2.0-flash` and confirming the chain recovers to real advice with `error=None`. The default first pick is `gemini-flash-lite-latest` (lite tier has the most free headroom; a one-line recommendation doesn't need more). The "quota exhausted" message now only appears if **every** model in the chain is dry. Note `gemini-2.5-flash`/`gemini-2.5-flash-lite` return `404` on this key (not valid free-tier ids) — that's why they're not in the chain.

### 5.10b ElevenLabs voiceover for the AI advisor

The Gemini advice is now also **read aloud** via ElevenLabs text-to-speech, so the advisor speaks its recommendation instead of only printing it. Kept as a strict enhancement layered on top of §5.10 — the text advice always shows first and the audio follows, so a voice failure never blocks the recommendation.

- `ui/api/voice.py` mirrors `advisor.py`'s contract exactly: soft-imports the ElevenLabs SDK (`from elevenlabs.client import ElevenLabs`) and checks for `ELEVENLABS_API_KEY`, so a missing package/key/quota degrades to a clear error string (advice stays text-only) rather than crashing. `synthesize(text)` returns MP3 bytes (`output_format="mp3_44100_128"` — plays natively in every browser) or a typed error, with a `MAX_CHARS=600` clamp so a malformed request can't run up the per-character bill.
- `ui/api/main.py` adds `POST /api/advisor/voice` — **stateless on purpose**: the frontend passes back the exact advice text it just received from `/api/advisor/feedback`, so there's no shared `_last_advice` server state to race between clients. The synth runs on `asyncio.to_thread` (like every other blocking call — see §5.9) so it never stalls the event loop. `/api/advisor/feedback` now also returns a `voice_available` boolean (true iff a key is configured) so the frontend skips the voice round-trip entirely when ElevenLabs isn't set up, instead of probing and eating a 503.
- Frontend (`ui/dashboard/js/app.js`): after the advice text renders, `speakAdvice()` fetches the MP3, plays it via an `Audio` element (autoplay is allowed because it's initiated by the user's "Ask AI"/"Replay" click — a genuine user gesture), and reveals a **🔊 Replay** button. Object URLs are revoked on replace to avoid leaks; a pulsing animation marks the speaking state.
- **Free-tier voice gotcha (verified live):** ElevenLabs free accounts **cannot** use premade "library" voices (Rachel `21m00Tcm4TlvDq8ikWAM`, etc.) via the API — those return **HTTP 402 `paid_plan_required`** ("Free users cannot use library voices via the API"). The default `ELEVENLABS_VOICE_ID` is therefore **`XrExE9yKIg1WjnnlVkGX` ("Matilda", professional)**, one of the ~21 voices the free account actually lists via `voices.get_all()` and which returns a real MP3 (confirmed `ID3`-header bytes). If you swap keys, list that account's available voices first rather than assuming a hardcoded id works. `voice.py::_format_error` maps 401/402/429 to specific, actionable messages.
- Model: `eleven_multilingual_v2` by default (`ELEVENLABS_MODEL`), chosen for quality and English/Spanish coverage.
...
- [x] Set `GEMINI_API_KEY` to actually get AI advisor responses — **RESOLVED**: key + `GEMINI_MODEL=gemini-flash-latest` set in the (gitignored) `.env`, verified returning live advice. See §5.10.
- [x] ElevenLabs voiceover reading the Gemini advice aloud — **RESOLVED**: `ELEVENLABS_API_KEY` set in `.env`, `ui/api/voice.py` + `POST /api/advisor/voice`, verified returning real MP3 audio. See §5.10b.
