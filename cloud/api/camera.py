"""MJPEG camera stream — mock frames for dev; swap for Pi/QNX capture later."""
...
def _render_mock_frame(frame_idx: int, width: int = 640, height: int = 480) -> bytes:
    ...
    if Image is None:
        # Minimal valid JPEG header fallback (1x1 black pixel)
        return (
    ...
    base_r, base_g, base_b = _state.color_baseline
    ...
    # Erlenmeyer flask shape (simplified)
    flask_top = 120
    ...
    # Culture medium — color drifts over time
    ...
    # Sample region indicator (fixed ROI for color metric)
    ...
    status_colors = {"HEATING": (220, 60, 60), "STABLE": (60, 200, 100), "COOLING": (60, 140, 220)}
    draw.text((50, 50), f"BioReact-Pi  |  {status}", fill=status_colors.get(status, (200, 200, 200)))
